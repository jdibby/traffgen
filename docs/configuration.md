...
| **Concurrent requests** | `_run_head_batch` (used by `https`, `ads`, `ai-browse`, `malware-agents`, etc.) uses **3 concurrent workers** with 0.2–0.6 s random jitter between each request submission. |
...
| *(Hagezi pro blocklist)* | `ads` | 300k+ domains fetched at runtime; falls back to `ad_endpoints` in `endpoints.py` if unreachable |
...
| `malware_endpoints` | `malware-agents` | Malware-category test URLs (WICAR, AMTSO, Google Safe Browsing test domains) |
| `c2_user_agents` | `malware-agents`, `c2-beacon` | C2 framework and malware family UAs (Cobalt Strike, Meterpreter, Empire, DarkComet, QuasarRAT, etc.) |
