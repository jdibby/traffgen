...
Traffgen ships **57 test suites** covering every major protocol and security control category. Use `--suite=<name>` to run a specific suite, or `--suite=all` to run every suite in shuffled order.
...
| 🗺️ `nmap` | Two-phase nmap scan against explicitly authorized public hosts (`scanme.nmap.org`, `testmyids.com`, `juice-shop.herokuapp.com`). Phase 1: port scan covering ports 1–1024. Phase 2: NSE script scan with `--script=default,vuln,exploit,malware,auth` — the ~80 scripts that produce security-relevant IDS/IPS triggers (CVE detection, auth bypass, malware indicators). Hosts scanned in parallel (up to 3 workers) for bounded wall-clock time. Tests whether IDS/IPS detects port-scan patterns and CVE-detection signatures. |
...
| 📶 `iperf3` | Runs a **TCP bidirectional** (`--bidir`) bandwidth test against public iperf3 servers on port 5201. Measures upload and download simultaneously in a single 10-second run. Size controls how many servers are tested: XS=1, S=2, M=3, L=4, XL=all 10. Connects timeout within 5 seconds so unavailable servers are skipped quickly. Validates egress TCP port 5201 and bulk flow detection. |
| 🕵️ `ips-ua` | Sends **~260 malicious/suspicious HTTP User-Agent strings** against `testmyids.com` and `scanme.nmap.org`. Sourced from the Emerging Threats open ruleset (`emerging-user_agents.rules`), mitchellkrogza/nginx-ultimate-bad-bot-blocker, mthcht/awesome-lists, and public malware reports. Categories: C2 frameworks (Cobalt Strike, Metasploit, Empire, Sliver), RATs/trojans (WHCC, BlackSun, Vertexnet, dozens more), modern infostealers (BunnyLoader, LokiBot, LemonDuck, Raccoon), vulnerability scanners (Nikto, sqlmap, Nessus, OpenVAS, Nuclei, ZGrab), credential-attack tools (Rubeus, Evilginx, AADInternals), bad bots, PowerShell/scripting indicators, cryptominer UAs (XMRig, LemonDuck), exploit-kit indicators, headless browsers, and AiTM frameworks. Validates that IDS/IPS / SASE fires on User-Agent–based signatures. Size controls sample size (XS=10, S=30, M=60, L=all). |
| 🔓 `cve-probe` | Sends **31 CVE-matched HTTP exploit probes** against `testmyids.com` and `scanme.nmap.org`. Probe categories: Remote Code Execution via headers (Log4Shell CVE-2021-44228/45046, Shellshock CVE-2014-6271, Struts2 CVE-2017-5638, Spring4Shell CVE-2022-22965, Confluence OGNL CVE-2022-26134, ActiveMQ CVE-2023-46604); Path traversal / LFI (Apache 2.4.49/50 CVE-2021-41773/42013, Pulse Secure CVE-2019-11510, F5 BIG-IP CVE-2020-5902, Fortinet CVE-2018-13379); Auth bypass (F5 iControl CVE-2022-1388, Cisco IOS XE CVE-2023-20198, SharePoint CVE-2019-0604); File-based byte patterns (Office Equation Editor CVE-2017-11882, HTA template CVE-2017-0199, MSCOMCTL CVE-2012-0158); Protocol-specific (EternalBlue CVE-2017-0144, ProxyLogon CVE-2021-26855, Zerologon CVE-2020-1472); generic injection (XXE, SSTI, PHP/China Chopper webshells); scanner UA probes (Nikto, sqlmap, Nessus, OpenVAS, Nuclei, ZGrab). Payloads target safe test hosts only — connections are refused/reset; no exploitation occurs. Validates IPS CVE signature detection on SASE/NGFW platforms. Size controls sample size (XS=5, S=10, M=20, L=all). |
| ⛏️ `crypto-mining` | **Stratum protocol handshakes + DNS lookups** for well-known mining pools (Monero/XMR: SupportXMR, C3Pool, MoneroOcean, HashVault, Nanopool; NiceHash; BTC: SlushPool, F2Pool; ETH-style: Ethermine, 2Miners). Stratum probes send a `mining.subscribe` JSON frame then close immediately — no mining work is accepted. DNS lookups query pool hostnames against threat-intel feeds. Validates Snort/Suricata ET COINMINER category signatures, stratum protocol DPI, and crypto-mining DNS block-list detection on any NGFW, IDS/IPS, or SASE platform. `blocked` = firewall RST or NXDOMAIN; `dropped` = silent firewall drop; `allowed` = pool reachable (control gap). Size controls number of probes per vector (XS=3, S=5, M=8, L=all). |
| 🔒 `ransomware` | **Ransomware pre-encryption traffic patterns** across three vectors. (1) DNS lookups for sinkholed/seized ransomware C2 domains: WannaCry kill-switch (Marcus Hutchins sinkhole), REvil victim portals (DOJ seizure 2022), Conti recovery site (group disbanded 2022), DarkSide (CISA AA21-131A), Maze, DoppelPaymer, Cl0p, Dharma/Phobos — all trigger NGFW and DNS-security ransomware-category feeds. (2) Geo-IP check-in: GET requests to `api.ipify.org`, `ipinfo.io`, `ifconfig.me` with ransomware-family UAs — the automated country-check every major ransomware family performs before encrypting (LockBit, ALPHV, Conti, REvil skip-list logic). (3) C2 HTTP beacons: POST/GET to `httpbin.org` and `postman-echo.com` using ransomware URI patterns (`/gate.php`, `/is_alive`, `/token`, `/report`) and observed implant UAs (Go-http-client, python-requests, WinHTTP, PowerShell). All HTTP targets are public echo/IDS-test services only. Validates DNS reputation, URI-pattern, UA-based, and behavioural anomaly detection rules on any NGFW/SASE/NDR platform. Size controls probes per vector (XS=3+3, S=5+6, M=8+10, L=all). |
