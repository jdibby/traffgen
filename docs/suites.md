...
| 🕵️ `lateral-movement` | Two-phase east-west reconnaissance against the Docker **host's physical LAN**. Phase 1: nmap `-sn` ping sweep of the /24 to enumerate live hosts. Phase 2: port scan every discovered host on 12 lateral-movement ports: SSH (22), Kerberos (88), WMI/RPC (135), NetBIOS (137-139), LDAP (389), SMB (445), LDAPS (636), RDP (3389), WinRM HTTP/HTTPS (5985/5986). See [Deployment Guide](deployment.md#lateral-movement-networking) for required network setup. |
