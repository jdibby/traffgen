...
| 📞 `voip` | Two-phase **VoIP/WebRTC signaling simulation**. Phase 1: STUN Binding Requests (UDP/3478 + UDP/19302) to 15 public STUN servers used by Zoom, Google Meet, Teams, and open-source SIP stacks — generates the UDP media-negotiation traffic that NGFW app-ID engines classify as "VoIP" or "WebRTC". Phase 2: SIP OPTIONS probes (UDP/5060) to 12 public SIP registrars (Linphone, SIPgate, AntiSIP, Zadarma, VoIP.ms, Callcentric) — the standard SIP "ping" that exercises SIP parser logic and triggers "SIP" / "VoIP-signaling" app-ID signatures. No real calls are placed. |
| 🎥 `ucaas` | HEAD requests to **30+ UCaaS and cloud-collaboration signaling URLs** across 10 platforms: Zoom, Microsoft Teams/Skype for Business, Cisco WebEx, Google Meet/Duo, Slack (huddles), RingCentral, 8x8, GoTo Meeting, Discord (voice channels), WhatsApp, Apple FaceTime, Vonage, Twilio, and Jitsi. NGFW/SASE platforms (Palo Alto, Cato Networks, Zscaler, Prisma Access) classify these as "video-conferencing", "voice-over-ip", or "UCaaS" application categories. Validates app-ID and URL-filtering policies without placing real calls. |
