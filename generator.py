...
def nmap_1024os():
    ...
    for ip in nmap_endpoints:
        cmd = 'nmap -Pn -p 1-1024 %s -T3 --randomize-hosts --script-args http.useragent "Mozilla/5.0 (Windows NT 10.0; WOW64; Trident/7.0; rv:11.0) like Gecko" -debug' % ip
        print (Fore.BLACK)
        print (Back.GREEN + "##############################################################")
        print (Style.RESET_ALL)
...
def nmap_cve():
    random.shuffle(nmap_endpoints)
    for ip in nmap_endpoints:
        cmd = 'nmap -sV --script=ALL %s -T3 --randomize-hosts --script-args http.useragent "Mozilla/5.0 (Windows NT 10.0; WOW64; Trident/7.0; rv:11.0) like Gecko" -debug' % ip
        print (Fore.BLACK)
        print (Back.GREEN + "##############################################################")
        print (Style.RESET_ALL)
