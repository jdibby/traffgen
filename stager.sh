...
echo "                    llm-dlp ids-sigs ips-ua cve-probe waf-attack log4shell nmap web-scanner"
...
echo "                    blocklist-probe snmp voip ucaas iperf3"
echo ""
...
if [ "$_CFG_WEBUI" -eq 1 ]; then
    ...
    if [ -n "$_CREDS_PW" ]; then
        ...
        echo "  ${BOLD}${GREEN}   Dashboard credentials  --  save these now${RESET}"
        echo "  ${BOLD}${GREEN}=================================================${RESET}"
        echo "  ${BOLD}   Username : traffadmin${RESET}"
        echo "  ${BOLD}   Password : ${GREEN}${_CREDS_PW}${RESET}"
        echo ""
        echo "  You will be prompted to change this password on first login."
        echo "  To reset credentials: remove and recreate the container."
        echo "  ${BOLD}${GREEN}=================================================${RESET}"
    fi
else
    echo ""
